      ...
      <div className="w-full md:w-[75vw] p-2 pb-3 flex items-center gap-2 lg:justify-between">
        ...
        <div className="w-full lg:w-3/5 flex items-end gap-2">
            ...
            {variant !== undefined && (
              <p
                className={`text-xs sm:text-sm lg:text-base ${
                  isMaxValid ? 'text-black' : 'text-destructive'
                }`}
              >{`Stock: ${variant.stock}${isMaxValid ? '' : '!'}`}</p>
            )}
            <QuantityController
              inputValue={quantity}
          ...
          <Button
            variant={'tertiary'}
            size={'customBlank'}
            className="px-1 py-2 w-full h-11"
          >
          ...
          <Button
            variant={'default'}
            size={'customBlank'}
            className="px-1 py-2 w-full h-11 text-base lg:text-lg"
          >
            Buy now
          </Button>
        </div>
